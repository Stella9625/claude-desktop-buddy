...
static void applyBrightness() { M5Axp.ScreenBreath(20 + brightLevel * 20); }
...
static void beep(uint16_t freq, uint16_t dur) {
  if (settings().sound) M5Beep.tone(freq, dur);
}
...
static void clockRefreshRtc() {
  ...
  _clkLastRead = millis();
  _onUsb = M5Axp.GetVBusVoltage() > 4.0f;
...
void drawInfo() {
  ...
  if (infoPage == 0) {
    ...
    int vBat_mV = (int)(M5Axp.GetBatVoltage() * 1000);
    int iBat_mA = (int)M5Axp.GetBatCurrent();
    int vBus_mV = (int)(M5Axp.GetVBusVoltage() * 1000);
    ...
    ln("  temp     %dC", (int)M5Axp.GetTempInAXP192());
